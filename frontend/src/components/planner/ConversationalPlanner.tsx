...
export default function ConversationalPlanner({
  ...
  }, [step, isOpen, selectedPick?.destination]);
  ...
  return (
    <div className="planner-floating-wrapper">
      {/* Closed Floating Launcher Button */}
      {!isOpen && (
        <button
          type="button"
          className="planner-floating-launcher"
          onClick={() => setIsOpen(true)}
          aria-label="Open VoyageAI Travel Assistant"
        >
          <span className="launcher-icon">✈️</span>
          <span className="launcher-title">VoyageAI Assistant</span>
          {selectedPick && (
            <span className="launcher-destination">{selectedPick.destination}</span>
          )}
        </button>
      )}

      {/* Open Floating Chat Assistant Panel */}
      {isOpen && (
        <div className="planner-container" data-testid="conversational-planner">
          ...
          <div className="planner-header">
            <div className="planner-header-title">
              ...
              <div>
                ...
                {selectedPick ? (
                  <p>
                    Planning for <strong>{selectedPick.destination}</strong> ({selectedPick.category})
                  </p>
                ) : (
                  <p>Select a destination recommendation above to begin</p>
                )}
              </div>
            </div>
          ...
          <div className="chat-messages" data-testid="chat-messages">
            {selectedPick ? (
              ...
              <div className="message-bubble message-bot">
                Welcome to VoyageAI! 🌟 Pick any destination recommendation above (Smart Picks, Hidden Gems, or Experiences) to start building your custom itinerary.
              </div>
            )}
          ...
          <div className="planner-controls" data-testid="planner-controls">
            {!selectedPick ? (
              <div className="no-pick-prompt">
                <span>👈 Select a destination card on the page to customize your trip details.</span>
              </div>
            ) : (
              <>
                {submitError && (
                ...
                {step === "TRAVELLERS" && (
                  <div className="control-group" data-testid="form-travellers">
                    <span className="control-label">Enter number of travellers:</span>
                    ...
                    >
                      <input
                        type="text"
                        className="planner-input"
                        placeholder="e.g. 3, or 'me and 2 friends'"
                        value={travellerInput}
                        onChange={(e) => setTravellerInput(e.target.value)}
                        required
                      />
                      <button type="submit" className="planner-primary-btn" style={{ padding: "8px 14px" }}>
                        Submit
                      </button>
                    </form>
                ...
                {step === "TRAVELLERS_CONFIRM" && (
                  <div className="control-group" data-testid="form-travellers-confirm">
                    <span className="control-label" style={{ color: "#a78bfa" }}>
                      Got it — <strong>{parsedTravellers}</strong> traveller(s). Is that correct?
                    </span>
                    <div className="controls-actions" style={{ justifyContent: "flex-start", marginTop: 6 }}>
                      <button
                        type="button"
                ...
                {step === "BUDGET" && (
                  <form onSubmit={handleBudgetSubmit} className="control-group" data-testid="form-budget">
                    ...
                    <input
                      type="text"
                      className="planner-input"
                      placeholder="e.g. ₹30,000 or '40k excluding flights'"
                      value={budgetInput}
                ...
                {step === "SPECIAL_REQ" && (
                  <div className="control-group" data-testid="form-special-req">
                    ...
                    <textarea
                      className="planner-input"
                      rows={2}
                      placeholder="e.g. Vegetarian food only, travelling with seniors"
